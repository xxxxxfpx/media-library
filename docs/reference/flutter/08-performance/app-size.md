> 原文链接: [https://docs.flutter.dev/perf/app-size](https://docs.flutter.dev/perf/app-size)
---
...
A default release build, such as one created by`flutter build apk`or`flutter build ios`, is built to conveniently assemble your upload package
...
Follow the Google[Play Console's instructions](https://support.google.com/googleplay/android-developer/answer/9302563?hl=en)for checking app download and
                  install sizes.

Produce an upload package for your application:

`flutter build appbundle`
Log into your[Google Play Console](https://play.google.com/apps/publish/). Upload your application binary by drag
...
Sign and export the IPA. The exported directory contains`App Thinning Size Report.txt`with details about your projected
...
In this example, the app has an approximate
                  ...
                  installation size of 13.7 MB on an iPhone12,1 ([Model ID / Hardware
                    number](https://en.wikipedia.org/wiki/List_of_iOS_devices#Models)for iPhone 11)
                  and iPhone11,8 (iPhone XR) running iOS 13.0.

To measure an iOS app exactly,
                  you have to upload a release IPA to Apple's
                  App Store Connect ([instructions](https://docs.flutter.dev/deployment/ios))
                  and obtain the size report from there.
                  IPAs are commonly larger than APKs as explained
                  in[How big is the Flutter engine?](https://docs.flutter.dev/resources/faq#how-big-is-the-flutter-engine), a
                  section in the Flutter[FAQ](https://docs.flutter.dev/resources/faq).
...
1. The tool compiles Dart in a way that records code size usage of Dart
                    packages.
1. The tool displays a high level summary of the size breakdown
...
In addition to analyzing a single build, two builds can also be diffed by
                  ...
                  Check out the[DevTools documentation](https://docs.flutter.dev/tools/devtools/app-size)for details.
...
For further information on using the DevTools app size tool,
                  check out the[DevTools documentation](https://docs.flutter.dev/tools/devtools/app-size).
...
When building a release version of your app,
                  ...
                  For an example of using this tag, see[Obfuscating Dart code](https://docs.flutter.dev/deployment/obfuscate).
