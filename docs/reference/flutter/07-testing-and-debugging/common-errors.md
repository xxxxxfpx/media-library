> 原文链接: [https://docs.flutter.dev/testing/common-errors](https://docs.flutter.dev/testing/common-errors)
---
...
This page explains several frequently-encountered Flutter
                  ...
                  Feel free to[open an issue](https://github.com/flutter/website/issues/new/choose)or[submit a pull request](https://github.com/flutter/website/pulls)to
...
- [Flutter errors demystified](https://medium.com/@hpatilabhi10/flutter-errors-demystified-red-screen-errors-vs-debug-console-errors-acb3b8ed2625)by Abishek
- [Understanding and addressing the grey screen in Flutter](https://medium.com/@LordChris/understanding-and-addressing-the-grey-screen-in-flutter-5e72c31f408f)by Christopher Nwosu-Madueke
- [Flutter stuck on white screen](https://www.dhiwise.com/post/flutter-stuck-on-white-screen-understanding-and-fixing)by Kesar Bhimani
...
In the above example,
                  the`Column`tries to be wider than the space the`Row`(its parent) can allocate to it, causing an overflow error.
...
"*To perform layout, Flutter walks the render tree in a depth-first traversal
                    andpasses down size constraintsfrom parent to child… Children respond bypassing up a sizeto their parent object within the constraints the parent
                    established.*" –[Flutter architectural overview](https://docs.flutter.dev/resources/architectural-overview#layout-and-rendering)

In this case, the`Row`widget doesn't constrain the
                  size of its children, nor does the`Column`widget.
                  Lacking constraints from its parent widget, the second`Text`widget tries to be as wide as all the characters
                  it needs to display. The self-determined width of the`Text`widget then gets adopted by the`Column`, which
...
Another way is to wrap the`Column`in a`Flexible`widget
                  ...
                  with a`flex`factor of 1.0, as[its source code](https://github.com/flutter/flutter/blob/c8e42b47f5ea8b5ff7bf2f2b0a2a8e765f1aa51d/packages/flutter/lib/src/widgets/basic.dart#L5166-L5174)shows.
                  To further understand how to use the`Flex`widget in Flutter layouts,
                  check out[this 90-second Widget of the Week video](https://www.youtube.com/watch?v=CI7x0mAZiY0)on the`Flexible`widget.
...
- [How to debug layout issues with the Flutter Inspector](https://blog.flutter.dev/how-to-debug-layout-issues-with-the-flutter-inspector-87460a7b9db#738b)
...
Usually, the issue is related to violation of box constraints,
                  ...
                  in Flutter on the[Understanding constraints](https://docs.flutter.dev/ui/layout/constraints)page.
...
The error is often caused when a`ListView`(or other kinds of scrollable widgets such as`GridView`)
...
This error occurs, for example, when a`Row`contains a`TextFormField`or a`TextField`but the latter has
                  no width constraint.
...
| --- | --- |
| Flexible | Row,Column, orFlex |
| Expanded(a specializedFlexible) | Row,Column, orFlex |
| Positioned | Stack |
| TableCell | Table |
...
A common scenario where this error occurs is when
                  attempting to trigger a`Dialog`from within the`build`method. This is often motivated by the need to
...
This code doesn't make an explicit call to`setState`,
                  ...
                  The`build`method isn't the right place to call`showDialog`because`build`can be called by the
...
For more information and to learn how to fix,
                  check out the following video on[PrimaryScrollController](https://api.flutter.dev/flutter/widgets/PrimaryScrollController-class.html):
...
Unless stated otherwise, the documentation on this site reflects Flutter 3.41.5. Page last updated on 2026-05-05.[View source](https://github.com/flutter/website/blob/main/sites/docs/src/content/testing/common-errors.md)or[report an issue](https://github.com/flutter/website/issues/new?template=1_page_issue.yml&page-url=https://docs.flutter.dev/testing/common-errors&page-source=https://github.com/flutter/website/blob/main/sites/docs/src/content/testing/common-errors.md).
