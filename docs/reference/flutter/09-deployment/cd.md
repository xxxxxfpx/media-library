> 原文链接: [https://docs.flutter.dev/deployment/cd](https://docs.flutter.dev/deployment/cd)
---
...
[fastlane](https://docs.fastlane.tools)is an open-source tool suite to automate releases and deployments
                  for your app.
...
1. Install fastlane`gem install fastlane`or`brew install fastlane`.
                    Visit the[fastlane docs](https://docs.fastlane.tools)for more info.
...
`FLUTTER_ROOT`
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)`flutter build appbundle`; and
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)`flutter build ipa`.

`flutter build appbundle`
`flutter build ipa`
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)In your`[project]/android`directory, run`fastlane init`.
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)In your`[project]/ios`directory,
                         run`fastlane init`.
...
`fastlane init`
`Appfile`
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)Check that`package_name`in`[project]/android/fastlane/Appfile`matches your package name in AndroidManifest.xml.
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)Check that`app_identifier`in`[project]/ios/fastlane/Appfile`also matches Info.plist's bundle identifier. Fill in`apple_id`,`itc_team_id`,`team_id`with your respective account info.
...
`itc_team_id`
`team_id`
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)Follow the[Supply setup steps](https://docs.fastlane.tools/getting-started/android/setup/#setting-up-supply)and ensure that`fastlane supply init`successfully syncs data from your
                         Play Store console.*Treat the .json file like your password and do not check
                           it into any public source control repositories.*
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)Your iTunes Connect username is already
...
`apple_id`
`FASTLANE_PASSWORD`
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)Follow the[Android app signing steps](https://docs.flutter.dev/deployment/android#signing-the-app).
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)On iOS, create and sign using a
...
`Fastfile`
- ruby@copy_button data={"buttonText":null,"classes":[],"title":"Copy code to clipboard"}/@copy_button

![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)On Android, follow the[fastlane Android beta deployment guide](https://docs.fastlane.tools/getting-started/android/beta-deployment/).
                          Your edit could be as simple as adding a`lane`that calls`upload_to_play_store`.
                          Set the`aab`argument to`../build/app/outputs/bundle/release/app-release.aab`to use the app bundle`flutter build`already built.
...
`flutter build`
![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)On iOS, follow the[fastlane iOS beta deployment guide](https://docs.fastlane.tools/getting-started/ios/beta-deployment/).
...
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)`flutter build appbundle`.
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)`flutter build ipa`.

`flutter build appbundle`
`flutter build ipa`
- ![Android](https://docs.flutter.dev/assets/images/docs/cd/android.png)`cd android`then`fastlane [name of the lane you created]`.
- ![iOS](https://docs.flutter.dev/assets/images/docs/cd/ios.png)`cd ios`then`fastlane [name of the lane you created]`.
...
- Remove the`json_key_file`field from`Appfile`and store the string
                            ...
                            Read the environment variable directly in your`Fastfile`.@copy_button data={"buttonText":null,"classes":[],"title":"Copy code to clipboard"}/@copy_button
- Serialize your upload key (for example, using base64) and save it as
                            ...
                            system during the install phase withbash@copy_button data={"buttonText":null,"classes":[],"title":"Copy code to clipboard"}/@copy_button
...
- The CI system needs access to your distribution certificate.
                            fastlane's[Match](https://docs.fastlane.tools/actions/match/)system is
                            recommended to synchronize your certificates across machines.

`FASTLANE_PASSWORD`
It's recommended to use a Gemfile instead of using an indeterministic`gem install fastlane`on the CI system each time to ensure the fastlane
                      dependencies are stable and reproducible between local and cloud machines.
                      However, this step is optional.

`gem install fastlane`
- In both your`[project]/android`and`[project]/ios`folders, create a`Gemfile`containing the following content:@copy_button data={"buttonText":null,"classes":[],"title":"Copy code to clipboard"}/@copy_button
...
`ANDROID_SDK_ROOT`
`osx_image: xcode9.2`
- Run`flutter build appbundle`or`flutter build ios --release --no-codesign --config-only`,
...
[Xcode Cloud](https://developer.apple.com/xcode-cloud)is a continuous integration and delivery service for building,
...
Xcode Cloud recognizes[custom build scripts](https://developer.apple.com/documentation/xcode/writing-custom-build-scripts)that can be
                  used to perform additional tasks at a designated time. It also includes a set
                  of[predefined environment variables](https://developer.apple.com/documentation/xcode/environment-variable-reference), such as`$CI_WORKSPACE`, which is the
...
An[Xcode Cloud workflow](https://developer.apple.com/documentation/xcode/xcode-cloud-workflow-reference)defines the steps performed in the CI/CD process
...
For your app's iOS variant, it's reasonable that you would want Xcode Cloud to
                  ...
                  modified either the Dart or iOS source files within the`lib\`and`ios\`directories.
...
`1`
`Next Build Number`
Check out[Setting the next build number for Xcode Cloud builds](https://developer.apple.com/documentation/xcode/setting-the-next-build-number-for-xcode-cloud-builds#Set-the-next-build-number-to-a-custom-value)for more
                  information.

Unless stated otherwise, the documentation on this site reflects Flutter 3.41.5. Page last updated on 2026-05-05.[View source](https://github.com/flutter/website/blob/main/sites/docs/src/content/deployment/cd.md)or[report an issue](https://github.com/flutter/website/issues/new?template=1_page_issue.yml&page-url=https://docs.flutter.dev/deployment/cd&page-source=https://github.com/flutter/website/blob/main/sites/docs/src/content/deployment/cd.md).
