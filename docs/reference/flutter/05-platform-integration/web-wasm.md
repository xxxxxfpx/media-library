> 原文链接: [https://docs.flutter.dev/platform-integration/web/wasm](https://docs.flutter.dev/platform-integration/web/wasm)
---

Flutter is back at Google I/O on May 19-20![Register now](https://io.google/2026/?utm_source=flutter&utm_medium=embedded_marketing&utm_campaign=flutter)

Flutter and Dart support[WebAssembly](https://webassembly.org/)as a compilation target when building
...
Try the default template[sample app](https://docs.flutter.dev/reference/create-new-app),
                  or choose any Flutter application
                  that has been migrated to be[compatible with Wasm](#js-interop-wasm).

### Modify the index page

Make sure your app's`web/index.html`is updated to the latest[Flutter web app initialization](https://docs.flutter.dev/platform-integration/web/initialization)for Flutter 3.22 and later.
...
| Name | Value |
| --- | --- |
| Cross-Origin-Embedder-Policy | credentiallessorrequire-corp |
...
To run a Flutter app that has been compiled to Wasm,
                  you need a browser that supports[WasmGC](https://github.com/WebAssembly/gc/tree/main/proposals/gc).

[Chromium and V8](https://chromestatus.com/feature/6062715726462976)support WasmGC since version 119.
                  Chrome on iOS uses WebKit, which doesn't yet[support WasmGC](https://bugs.webkit.org/show_bug.cgi?id=247394).
...
- **Why not Firefox?**Firefox versions 120 and later were previously able to run Flutter/Wasm, but
                    they're experiencing a bug that is blocking compatibility with Flutter's Wasm
                    renderer. Follow[this bug](https://bugzilla.mozilla.org/show_bug.cgi?id=1788206)for details.
- **Why not Safari?**Safari now supports WasmGC, but is experiencing a similar bug that is
                    blocking compatibility with Flutter's Wasm renderer.
                    Follow[this bug](https://bugs.webkit.org/show_bug.cgi?id=267291)for details.
...
To support compilation to Wasm, Dart has changed
                  ...
                  This prevents Dart code that uses`dart:html`or`package:js`from compiling to Wasm.
...
- [package:web](https://pub.dev/packages/web), which replaces`dart:html`(and other web libraries)
- [dart:js_interop](https://api.dart.dev/dart-js_interop/dart-js_interop-library.html), which replaces`package:js`and`dart:js`
